...
**Status:** IMPLEMENTED & MERGED — frozen as historical record (PR #2, squash-merged to main 2026-08-22 as `a08e4e2`; all 5 steps complete, final panel 4/4 CLEAN). Future architectural extension goes through a new TAD per `../../../agent-server-manager/CODE_REVIEW_PROCESS.md`.
...
4. **Runtime integration behind flag.** — **DONE 2026-08-21** (see Progress log).
...
5. **Flip default, keep parser fallback.** — **DONE 2026-08-22** (see Progress log).
...
- **2026-08-22** — **Step 5 executed.** Trait composition is the default; `?traitcompose=0` opts out to the legacy path, and any composition failure falls back automatically (console-warned). Full regression green including a forced-failure test (library fetch aborted → legacy fallback renders). Follow-up candidate noted for a future TAD: picker thumbnails still render via the heavy `renderTokenSVG` call and could compose from the library instead.
- **2026-08-21** — **Step 4 executed.** `Lib/TraitComposer.js` composes any token from the committed library via static fetches only — no wallet, no contract calls: the deployed hash corpus supplies the trait hash, so all 26,617 Avastars are instantly loadable under the flag. Layers = genes 5–11 (each an image with the four color styles injected, carrying name/gene/rarity metadata); backdrop (gene 4) is the stretch-to-fill background; `bg_color` extracted by regex (no cssjs dependency). MainScene tries composition first under `?traitcompose=1` with the legacy loader+parser as automatic fallback (console-warned); resize recomposes from the composer's fragment cache. `deploy.sh` ships `Traits/` + the hash corpus. Harness: composed path renders 8014 with correctly named layers ("Pigtails", "Right Wink"…), content-parity (whitespace-normalized) against the bundled reference file — the bundled `SVG/*.svg` files turn out to be pretty-printed saves of chain output, so byte-parity vs chain remains `validate-composition.js`'s job — and flag-off regression is clean. One discovery: composition makes the runtime *faster* than the legacy path for wallet users (no heavy `renderAvastar` eth_call at all).
